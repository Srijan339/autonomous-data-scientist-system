...
class DataCleaner:
    ...
    def fix_numeric_types(self):
        """
        Convert string numeric columns to float, PROTECTING Titanic categorical features
        """
        protected_cats = ["Sex", "Embarked", "Cabin"]
        print(f"🔒 Protecting categorical columns from numeric conversion: {protected_cats}")
        for col in self.df.columns:
            if self.df[col].dtype == "object" and col not in protected_cats:
                try:
